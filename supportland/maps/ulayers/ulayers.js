...
var uLayers = {
    ...
    OSM: {
        ...
        markerUrl: "http://www.openstreetmap.org/openlayers/img/marker.png",
        ...
        getTile: function (lonlat, zoom) {
            ...
            result.y = (Math.floor((1 - Math.log(Math.tan(lonlat.lat * Math.PI / 180)
                + 1 / Math.cos(lonlat.lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom)));
            result.z = zoom;
            return result;
        },
        ...
        getPixelCoords: function (lonlat, zoom) {
            ...
            result.y = ((1 - Math.log(Math.tan(lonlat.lat * Math.PI / 180)
                + 1 / Math.cos(lonlat.lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, zoom));
            return result;
        }
    },
...
uLayers.Map.prototype.recalcMarker = function (markerId) {
    ...
    o.style.left = Math.floor((this.map.clientWidth / 2) ) + "px";
    o.style.top = Math.floor((this.map.clientHeight / 2) ) + "px";
}
...
uLayers.Map.prototype.updateMap = function () {
    ...
    var t = Math.floor(uLayers.max(this.map.clientHeight / 256 + 1,
                                   this.map.clientWidth / 256 + 1) * 1.5) + 1;
